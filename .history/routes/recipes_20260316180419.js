...
router.post('/recipe/update/:id', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [name, difficulty, prepTime, cookTime, totalTime, serves, recipeLink, id]
    );
...
router.get('/suggestion', isAuthenticated, async (req, res) => {
  try {
    ...
    res.status(500).render('error', { message: 'Error loading suggestion page' });
  }
});
...
router.post('/api/suggestion', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      if (allRecipes.rows.length === req.session.suggestedRecipes.length) {
        return res.json({
          recipe: null,
          allSuggested: true,
          message: 'You have seen all recipes! Click continue to cycle through again.'
        });
      }
    ...
    res.status(500).json({ success: false, message: 'Error getting suggestion' });
  }
});
